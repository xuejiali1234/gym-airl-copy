...
P30_ENV = {
    ...
    "PROBE_GOAL_ABLATION_MODE": "normal",
...
EXPERIMENTS = [
    {
        "name": "AG0_P30_Full",
        "description": "Sanity control: full P30 config with normal attention and normal goal; used to verify this ablation entrypoint reproduces P30.",
        "env": {
            "PROBE_ATTENTION_ABLATION_MODE": "normal",
            "PROBE_GOAL_ABLATION_MODE": "normal",
        },
    ...
    {
        "name": "AG1_AttnZero_P30",
        "description": "P30 full safety/control config; keep attention architecture but zero SocialAttentionLayer output; goal remains normal.",
        "env": {
            "PROBE_ATTENTION_ABLATION_MODE": "zero",
            "PROBE_GOAL_ABLATION_MODE": "normal",
        },
    },
    {
        "name": "AG2_GoalZero_P30",
        "description": "P30 full safety/control config; keep 18-dim observation but zero the two goal dimensions; attention remains normal.",
        "env": {
            "PROBE_ATTENTION_ABLATION_MODE": "normal",
            "PROBE_GOAL_ABLATION_MODE": "zero",
        },
    },
    {
        "name": "AG3_AttnZero_GoalZero_P30",
        "description": "P30 full safety/control config; zero attention output and zero the two goal dimensions while preserving architecture and observation dim.",
        "env": {
            "PROBE_ATTENTION_ABLATION_MODE": "zero",
            "PROBE_GOAL_ABLATION_MODE": "zero",
        },
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run structure-preserving attention/goal ablations on the P30 base.")
    parser.add_argument(
        "--only",
        default="",
...
def main():
    ...
    print("[*] Attention/goal ablation sweep plan:")
    ...
    manifest_path = REPO_ROOT / "train_log" / f"attn_goal_ablation_manifest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    for idx, exp in enumerate(experiments, start=1):
        ...
        print(f"\n[*] Running ablation {idx}/{len(experiments)}: {exp['name']}")
        ...
        if result.returncode != 0:
            raise SystemExit(f"Ablation failed: {exp['name']} (exit={result.returncode})")

    print("\n[*] Attention/goal ablation sweep finished.")
